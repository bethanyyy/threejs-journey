...
const tick = () =>
{
    ...
    for (let i = 0; i < generated_cubes.length; i++)
    {
        ...
        let x_movement_cubes = mouseX * elapsedTimeMouse*.01

        // generated_cubes[i].position.y += Math.min(Math.max(y_movement_cubes * 3, -.1), .1)
        // generated_cubes[i].position.z += Math.min(Math.max(y_movement_cubes, -.7), .7,) * .5
        // generated_cubes[i].position.x += - Math.min(Math.max(x_movement_cubes, -.7), .7)
        ...
        // generated_cubes[i].position.x = Math.min(Math.max(generated_cubes[i].position.x, -100), 100)
    ...
    // group01.position.y = Math.min(Math.max(group01, -10), 10)
